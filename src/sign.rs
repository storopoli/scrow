...
mod tests {
    ...
    #[test]
    fn sign_dispute_tx_flow_with_multi_funding() {
        tracing_subscriber::registry()
            .with(fmt::layer())
            .with(EnvFilter::from_default_env())
            .init();
        ...
        // Fund a SegWit-v1 P2TR address from the npub.
        // Mine until maturity (101 blocks in Regtest).
        let funded_address_1 = npub_to_address(&npub_1, network).unwrap();
        let funded_address_2 = npub_to_address(&npub_1, network).unwrap();
        trace!(%funded_address_1, "Funded address");
        let coinbase_block_1 = btc_client
            .generate_to_address(COINBASE_MATURITY, &funded_address_1)
            .expect("must be able to generate blocks")
            .0
            .first()
            .expect("must be able to get the blocks")
            .parse::<BlockHash>()
            .expect("must parse");
        let coinbase_txid_1 = btc_client
            .get_block(coinbase_block_1)
            .expect("must be able to get coinbase block")
            .coinbase()
            .expect("must be able to get the coinbase transaction")
            .compute_txid();
        let coinbase_block_2 = btc_client
            .generate_to_address(COINBASE_MATURITY, &funded_address_2)
            .expect("must be able to generate blocks")
            .0
            .first()
            .expect("must be able to get the blocks")
            .parse::<BlockHash>()
            .expect("must parse");
        let coinbase_txid_2 = btc_client
            .get_block(coinbase_block_2)
            .expect("must be able to get coinbase block")
            .coinbase()
            .expect("must be able to get the coinbase transaction")
            .compute_txid();
        ...
        let funding_input_1 = OutPoint {
            txid: coinbase_txid_1,
            vout: 0,
        };
        let funding_input_2 = OutPoint {
            txid: coinbase_txid_2,
            vout: 0,
        };
        let inputs_1 = vec![TxIn {
            previous_output: funding_input_1,
            ..Default::default()
        }];
        let inputs_2 = vec![TxIn {
            previous_output: funding_input_2,
            ..Default::default()
        }];
        let outputs = vec![TxOut {
            value: *MULTISIG_AMOUNT,
        ...
        let unsigned_1 = Transaction {
            version: transaction::Version(2),
            input: inputs_1,
            output: outputs.clone(),
            lock_time: absolute::LockTime::ZERO,
        };
        let unsigned_2 = Transaction {
            version: transaction::Version(2),
            input: inputs_2,
            output: outputs.clone(),
            lock_time: absolute::LockTime::ZERO,
        };
        trace!(transaction=%consensus::serialize(&unsigned_1).as_hex(), "Unsigned funding transaction 1");
        trace!(transaction=%consensus::serialize(&unsigned_2).as_hex(), "Unsigned funding transaction 2");

        // Sign the first input using Sighashes
        let prevouts_1 = TxOut {
            value: *COINBASE_AMOUNT,
            script_pubkey: funded_address_1.script_pubkey(),
        };
        let prevouts_2 = TxOut {
            value: *COINBASE_AMOUNT,
            script_pubkey: funded_address_2.script_pubkey(),
        };
        let signed_1 = sign_resolution_tx(&unsigned_1, &nsec_1, prevouts_1);
        let signed_2 = sign_resolution_tx(&unsigned_2, &nsec_2, prevouts_2);
        trace!(transaction=%consensus::serialize(&signed_1).as_hex(), "Signed funding 1");
        trace!(transaction=%consensus::serialize(&signed_2).as_hex(), "Signed funding 2");

        // Test if the transactions are valid.
        let result = btc_client.send_raw_transaction(&signed_1);
        assert!(result.is_ok());
        let txid_1 = result.unwrap().txid().unwrap();
        assert_eq!(txid_1, signed_1.compute_txid());
        debug!(%txid_1, "Sent to the escrow address from transaction 1");
        let result = btc_client.send_raw_transaction(&signed_2);
        assert!(result.is_ok());
        let txid_2 = result.unwrap().txid().unwrap();
        assert_eq!(txid_2, signed_2.compute_txid());
        debug!(%txid_2, "Sent to the escrow address from transaction 1");
        // Mine 1 block to mine the transaction
        btc_client
            .generate_to_address(1, &funded_address_1)
            .unwrap();
        ...
        // Now let's move timelock_duration - 1 blocks and should pass
        btc_client
            .generate_to_address(timelock_duration as usize - 1, &funded_address_1)
            .unwrap();
        let result = btc_client.send_raw_transaction(&signed);
        assert!(result.is_ok());
    }
