...
mod tests {
    use bitcoin::{
        Amount, BlockHash, Network, OutPoint, TxIn, absolute::LockTime, consensus, hex::DisplayHex,
        transaction,
    };
    ...
    fn sign_collaborative_tx_flow() {
        ...
        let multisig_address = escrow_address(&npub_1, &npub_2, None, None, network).unwrap();
        ...
        println!(
            ...
            consensus::serialize(&signed).as_hex()
        );
